...
imsize = int(model_name[-3:])  # 336
args = args_parser.parse_args(args=[
                                    "--train_path", "database/Objects",
                                    "--test_path", "database/Data/test_1_"+scene_level,  # test_002
                                    "--output_dir", f"exps/{model_name}_cls_argmax_adapted_bs1024_"+scene_level+"4_gdino0.15t" + str(imsize) + "_" + tag,
                                    ])
...
adapter_type = "weight"
if use_adapter:
    input_features = 1024 #1024 #768, 1024, the vector dimension
    if adapter_type == "clip":
        ...
        adapter_args = 'PE-Core-L14-336_insDet_weight_04242025_temp_0.05_epoch_80_lr_0.001_bs_1024_vec_reduction_4'
        model_path = 'adapter_weights/adapter2FC/' + adapter_args + '_weights.pth'
        adapter = WeightAdapter(input_features, reduction=4).to(device)
...
output_dir = './object_pe_features'
json_filename = 'PE-Core-L14-336_original_cls.json' # 'PE-Core-L14-336_FFA.json', 'PE-Core-G14-448_original_cls.json'
...
gdino = GroundingDINOObjectPredictor(use_vitb=False, threshold=0.15) # 0.15 default
SAM = SegmentAnythingPredictor(vit_model="vit_h", device="cuda:1")
    ...
    for i in range(num_imgs):
        ...
        with torch.no_grad():
            if use_class_token:
                ...
                image_feature = get_features_PE_FFA(img, mask, encoder, preprocess, model_name, device=device).float()
            if use_adapter:
